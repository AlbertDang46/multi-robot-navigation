...
class RVAEP(nn.Module):
    ...
    def forward(self, x, x_map,pos=None,ego_index=None,fusion='no'):
        ...
        if fusion == 'late':
                ...
                #plot_ogm(x_map[0][0],'x_map.png')
                robot_num,b, seq_len, c, h, w = x.size()
                prediction_combined=torch.zeros_like(x_map[0]).to(x.device)
                ...
                for r in range(robot_num):
                    ...
                    T = get_transformation_matrix(rotation_matrices, (32, 32))
                    prediction = warp_affine(prediction, T, (32, 32))
                    # if r==ego_index:
                    #     plot_ogm(prediction[0],f'prediction_{ego_index}_{r}.png')
                    for bz in range(b):
                        if distance_check[bz]:
                            prediction_combined[bz]+=(prediction[bz])
                #plot_ogm(prediction_combined[0],f'combined_prediction_{ego_index}_{r}.png')
                return prediction,kl_loss_ego
