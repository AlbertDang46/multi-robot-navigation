...
from SOGMP_plus.CoBEVT.opv2v.opencood.utils.common_utils import torch_tensor_to_numpy
...
def regroup(dense_feature, record_len, max_len):
    ...
    regroup_feature : torch.Tensor
        B, L, C, H, W
    """
    cum_sum_len = torch.tensor(np.cumsum(torch_tensor_to_numpy(record_len))).long()
    ...
    for split_feature in split_features:
        ...
        split_feature = torch.cat([split_feature, padding_tensor],
                                  dim=0)
        #print(split_feature.shape,padding_tensor.shape)
        # 1, max_len*C, H, W
