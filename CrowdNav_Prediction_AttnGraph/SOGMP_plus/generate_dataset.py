...
# Define base paths
base_dir = 'SOGMP_plus/OGM-datasets/dataset_900_1'
...
# Load the original .npy files
lidar_data = np.load('3r3p_successful_lidar_seq_1.npy') 
vel_pos_data = np.load('3r3p_successful_vel_pos_seq_1.npy')
future_lidar_data =  np.load('3r3p_future_seq_1.npy')
...
traj_num,seq_len,robot_num,num_ray,_ = lidar_data.shape

lidar_data = lidar_data.reshape(traj_num*seq_len, robot_num, num_ray, 2)
new_vel_pos = new_vel_pos.reshape(traj_num*seq_len, robot_num, -1)
future_lidar_data = np.transpose(future_lidar_data, (0, 1, 3, 2, 4, 5))
future_lidar_data = future_lidar_data.reshape(traj_num*seq_len, robot_num, FUTURE_STEP, num_ray, 2)
