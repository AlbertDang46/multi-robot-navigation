...
class Lidar2d:
    def __init__(self, bitmap: np.ndarray, num_ray: int = 1000, sensor_range: int = -1, cell_length: float = 1):
    ...
    def get_raw_data(self, x, y) -> np.ndarray:
...
def merge_ogm(local_ogm, recieved_ogm, relative_position: List[float], cell_length:float = 1, trust_rate : float = 0.5) -> np.ndarray:
    assert trust_rate >= 0 and trust_rate <= 1
    #local_ogm = np.copy(local_ogm)
    rel_x = int(relative_position[0]/cell_length)
    rel_y = int(relative_position[1]/cell_length)
    ...
    for i in range(recieved_ogm.shape[1]):
        for j in range(recieved_ogm.shape[2]):
            if recieved_ogm[1,i,j] == -1 :
                continue
            x = i + rel_x + (local_ogm.shape[1] - recieved_ogm.shape[1])//2
            y = j + rel_y + (local_ogm.shape[2] - recieved_ogm.shape[2])//2
            if 0 <= x < local_ogm.shape[1] and 0 <= y < local_ogm.shape[2]:
                # If the cell in local is unknown, update the cell
                if local_ogm[1,x,y] == -1:
...
if __name__ == "__main__":
    ...
    lidar1 = lidar.get_raw_data(27,27)
    ogm1 = lidar.convert_to_bitmap(lidar1, 56)
    ogm1_ori = np.copy(ogm1)
    lidar2 = lidar.get_raw_data(37,37)
    ogm2 = lidar.convert_to_bitmap(lidar2, 48)
    merged_ogm = merge_ogm(ogm1, ogm2, [10,10])
