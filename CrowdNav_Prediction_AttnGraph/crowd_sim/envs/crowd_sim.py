...
class CrowdSim(gym.Env):
    ...
    def __init__(self):
        ...
        self.robots = None # a list of Robot instances, representing all robots in the environment
    ...
    def get_static_map(self, map_id):
    ...
    def configure(self, config):
        ...
        self.robot_num = config.sim.robot_num
        ...
        #print('set robot for this envs')
        for i in range(self.robot_num):
            rob_RL = Robot(config, 'robot')
            self.set_robot(robot=rob_RL)
        self.get_static_map(6)
    ...
    def generate_random_human_position(self, human_num):
        #print('enter crowd_sim/generate_random_human_position')
        """
    ...
    def detect_visible(self, state1, state2, robot1 = False, custom_fov=None, custom_sensor_range=None):
        ...
        if self.robots[0].kinematics == 'holonomic': #assume all robots have the same kinematics
        ...
        if custom_fov:
            ...
            if robot1:
                ...
                fov = self.human_fov
        # detect whether state1-2 cross the obstacle
        
        # Check if the line between state1 and state2 crosses an obstacle
        
        blk=False
        # if self.static_map is not None:
           
            
        #     x1, y1 = state1.px, state1.py
        #     x2, y2 = state2.px, state2.py
            
        #     x1=int(self.static_map_size / 2 + np.floor(x1 / self.cell_length))
        #     x2=int(self.static_map_size / 2 + np.floor(x2 / self.cell_length))
        #     y1=int(self.static_map_size / 2 + np.floor(y1 / self.cell_length))
        #     y2=int(self.static_map_size / 2 + np.floor(y2 / self.cell_length))
        ...
        #         #     cell_r = int(self.static_map_size / 2 + np.floor(rr[i] / self.cell_length))
        #         #     cell_c = int(self.static_map_size / 2 + np.floor(cc[i] / self.cell_length))
        #         #     rr[i]=cell_r-1
        #         #     cc[i]=cell_c-1
                
        #         # Check if any of the coordinates cross an obstacle
        #         if np.any(self.static_map[rr, cc] == 1):
                    
        #             blk=True  # Line of sight is blocked
            
            
        
        if np.abs(offset) <= fov / 2 and not blk:
            inFov = True
        ...
        if custom_sensor_range:
            ...
            if robot1:
                inSensorRange = dist <= state1.sensor_range #
            else:
                inSensorRange = True
        # if blk and inSensorRange:
        #     print("Obstacle detected, pausing for 10 seconds...")
        #     print("state1: ", state1.px, state1.py)
        #     print("state2: ", state2.px, state2.py)
        #     print(rr, cc)
        #     print(np.any(self.static_map[rr, cc] == 1))
    ...
    def get_num_human_in_fov(self,robot_index):
        ...
        for i in range(self.human_num):
            visible = self.detect_visible(self.robots[robot_index], self.humans[i], robot1=True) #enter crowd_sim/detect_visible
            if visible:
    ...
    def get_human_actions(self):
        ...
        for i, human in enumerate(self.humans):
            ...
            if self.robots[0].visible: #Flase
