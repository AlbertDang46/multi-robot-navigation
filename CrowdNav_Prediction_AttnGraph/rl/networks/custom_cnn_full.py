...
class CustomCNN(BaseFeaturesExtractor):
    ...
    def _forward_impl(self, map, goal):
        ###### Start of fusion net ######
        ...
        # x_ = copy.deepcopy(x[0]).cpu().detach().numpy()
        # import matplotlib.pyplot as plt
        # import os

        # # Create a directory to save the plots
        # save_dir = "feat_plots_layer_3"
        # os.makedirs(save_dir, exist_ok=True)

        # # Iterate over each channel (dim=1) of the tensor and save the plot
        # chann,height, width = x_.shape
        # for i in range(chann):
        #     plt.figure()  # Create a new figure for each plot
        #     plt.imshow(x_[i], cmap='gray')  # Plot the i-th channel of the first sample in the batch
    ...
    def forward(self, observations: torch.Tensor, goal) -> torch.Tensor:
        # preprocessing:
        return self._forward_impl(observations, goal)
    #
    # end of method
#
# end of class
