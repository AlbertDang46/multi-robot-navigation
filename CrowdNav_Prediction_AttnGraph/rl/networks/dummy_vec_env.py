...
class DummyVecEnv(VecEnv):
    ...
    def __init__(self, env_fns):
        ...
        self.buf_obs = { k: np.zeros((self.num_envs,)+(env.robot_num,)+ tuple(shapes[k]), dtype=dtypes[k]) for k in self.keys } #
        self.buf_dones = np.zeros((self.num_envs,), dtype=bool)
        #self.buf_rews  = np.zeros((self.num_envs,), dtype=np.float32)
        self.buf_rews = np.zeros((self.num_envs,env.robot_num), dtype=np.float32)
