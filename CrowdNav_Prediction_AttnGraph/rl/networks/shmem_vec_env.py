...
def _subproc_worker(pipe, parent_pipe, env_fn_wrapper, obs_bufs, obs_shapes, obs_dtypes, keys):
    ...
    def _write_obs(maybe_dict_obs):
        all_obs = {}
        for key in maybe_dict_obs[0].keys():
            all_obs[key] = []
            for ob in maybe_dict_obs:
                if key == 'visible_masks' or np.array(ob[key]).ndim>1:
                    all_obs[key].append(np.array(ob[key]))
                else:
                    all_obs[key].append([np.array(ob[key])])

            all_obs[key] = np.stack(all_obs[key])
        maybe_dict_obs = all_obs
